...
async def generateEmbed(ctx, bot, config, links, language, voltage, translator, python_version):
...
async def editEmbed(ctx, bot, config, links, language, voltage, translator, python_version, uptime):
    try:
    ...
    msg_embed.description = '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_versf', language), translator.translate('embed_fields', 'about_versv', language).format(config['version'], config['version_date']))
    if(dev == None):
        pass
    else:
        msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_devsf', language), translator.translate('embed_fields', 'about_devsv', language).format(dev.name))
    msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_statsf', language), translator.translate('embed_fields', 'about_statsv', language).format(len(bot.cache.servers), len(bot.users)))
    msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_uptimef', language), translator.translate('embed_fields', 'about_uptimev', language).format(uptime))
    msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_basedf', language), translator.translate('embed_fields', 'about_basedv', language).format(python_version(), voltage.__version__, sqlite3.sqlite_version))

    msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_hardwf', language), translator.translate('embed_fields', 'about_hardwv', language).format(cpuinfo.get_cpu_info()['brand_raw'], round(cpuinfo.get_cpu_info()['hz_advertised'][0] / 1000000, 2), ram, os_ver))
    ...
    if(len(links_str) > 0):
        msg_embed.description += '### {0}\r\n{1}\r\n'.format(translator.translate('embed_fields', 'about_linksf', language), links_str)
    msg_embed.description += '###### Copyright © 2022 Dmitry Tretyakov (aka. Tinelix)'
    return msg_embed

async def sendSlashMsg(ctx, bot, config, links, language, voltage, translator, python_version, uptime):
    await ctx.response.defer()
    msg_embed = await editEmbed(ctx, bot, config, links, language, voltage, translator, python_version, uptime)
    await ctx.send(embed=msg_embed)

async def sendRegularMsg(ctx, bot, config, links, language, voltage, translator, python_version, uptime):
    msg_embed = await generateEmbed(ctx, bot, config, links, language, voltage, translator, python_version)
    msg = await ctx.reply(" ", embed=msg_embed)
    msg_embed_2 = await editEmbed(ctx, bot, config, links, language, voltage, translator, python_version, uptime)
