...
async def generateEmbed(ctx, inst, config, disnake, translator, arg):
    if(ctx.message.author.id == config['dev_id']):  # only bot owner!
        try:
            ...
            msg_embed.set_author(name=str(translator.translate('embed_title', 'eval', inst.language)))
                ...
                for guild in bot.guilds:
                    ...
                    for member in guild.members:
                        if(str(member.status) == 'online' or str(member.status) == 'idle' or str(member.status) == 'dnd'):
                            online += 1
                    eval_r += '{0:03d}. {1}\r\n└────┤ {2: 4d} members │ {3: 4d} online │\r\n'.format(inst.bot.guilds.index(guild) + 1, guild.name, guild.member_count, online)
            elif("token" in arg):
            ...
            if(arg == 'guilds'):
                ...
                msg_embed.add_field(translator.translate('embed_fields', 'eval_codelf', inst.language), '```py\r\n{0}```'.format(arg), inline=False)
            msg_embed.add_field(translator.translate('embed_fields', 'eval_resulf', inst.language), '```py\r\n{0}```'.format(eval_r), inline=False)
